...
async def deploy(name: str, description: str, params: DeploymentParams, identifier = Depends(get_current_identifier), db: Session = Depends(gen_db)):
    entry = Deployment(
        name = name,
...
async def get(id: int, identifier = Depends(get_current_identifier), db: Session = Depends(gen_db)):
    return accessible(db.query(Deployment).filter(Deployment.id == id), identifier).first()

@deployment_router.delete("/{id}")
async def delete(id: int, identifier = Depends(get_current_identifier), db: Session = Depends(gen_db)):
