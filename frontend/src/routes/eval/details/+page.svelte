<script lang="ts">
    ...
    $: eval_results = (eval_entry && eval_entry.result != undefined && eval_entry.result != null) ? Object.entries(eval_entry.result).map(([key, value]) => ({
        name: key,
        value: value
    })) : [];
...
<Modal title="Confirm Deletion" bind:open={delete_modal} autoclose>
    <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Are you sure you want to delete?</p>
    <p class="text-base leading-relaxed text-red-600 dark:text-gray-400">
        After deletion, all related information of this task will be <span class="font-semibold">irrecoverable</span>.
    </p>
...
<div class="flex flex-row w-full">
    <div class="p-4 w-1/2">
        <div class="uppercase tracking-wide text-sm text-blue-600 font-semibold">{eval_entry.name}</div>
        ...
        <div class="mt-2">
            ...
            <span class="text-gray-600">{eval_entry.bits_and_bytes ? 'Yes' : 'No'}</span>
        </div>
        <div class="mt-2">
        ...
        <div class="mt-2">
            ...
            <span class="text-gray-600">{eval_entry.use_deepspeed ? 'Yes' : 'No'}</span>
        </div>
    </div>
    <div class="p-4 w-1/3">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            {#each eval_results as result}
                <div class="px-4 py-5 sm:px-6 flex justify-between border-t border-gray-200">
                    <h1 class="text-lg leading-6 font-medium text-gray-900">{
                        eval_index_full_name[result.name]
                        }</h1>
                    <span class="mt-1 max-w-2xl text-sm text-gray-500">{result.value}</span>
                </div>
            {/each}
        </div>
    {/if}
