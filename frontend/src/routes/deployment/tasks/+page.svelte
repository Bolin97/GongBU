<script lang="ts">
	...
	import { default_deployment_request_params, type DeploymentRequestParams } from "../../../class/DeploymentRequestParams";
	...
	let deployment_request_params: DeploymentRequestParams = default_deployment_request_params()
	$: {
		deployment_request_params.devices = use_devices == "auto" ? ["auto"] : use_devices.map((x) => x.toString());
		deployment_request_params.model_or_adpater_id = selected_model_id.length > 0 ? Number.parseInt(selected_model_id) : Number.parseFloat(selected_adapter_id);
	...
	<div class="w-full m-2">
		...
		<div class="flex flex-row-reverse gap-5 m-2">
			...
			{:else if current_step == 1}
				<Button disabled={selected_adapter_id.length == 0 && selected_model_id.length == 0} on:click={(_) => ++current_step}>下一步</Button>
			{:else if current_step == 2}
				<Button disabled={use_devices.length == 0} on:click={(_) => ++current_step}>下一步</Button>
			{:else}
