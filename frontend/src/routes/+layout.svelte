		...
		{#if !logging_in}
			<div class="w-full p-2">
				<form class="w-full max-w-sm m-auto">
					<div class="mb-4">
						<label for="username" class="block text-gray-700 text-sm font-bold mb-2">{t("root.username")}</label>
					...
					<div class="mb-6">
						<label for="password" class="block text-gray-700 text-sm font-bold mb-2">{t("root.password")}</label>
					...
					<div class="flex items-center justify-between">
						<button type="button" on:click={login_handle} class="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
							{t("root.login")}
						</button>
					</div>
				</form>
