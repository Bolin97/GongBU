...
{#if models.length != 0}
	<div class="grid grid-cols-2">
		{#each models as model}
			<div class="mx-4 my-2">
				<ModelCard modelId={model.id} showAdapters adapterAction baseModelNoCursorChange adapterNoCursorChange>
					<svelte:fragment slot="base-action">
						<button
							on:click={(_) =>
								goto(`/finetune/tasks?model_id=${model.id}`)}
							type="button"
						...
						<button
							on:click={(_) =>
								goto(`/deployment/tasks?model_id=${model.id}`)}
							type="button"
					...
					<svelte:fragment slot="adapter-action" let:adapter={adapter}>
						<button
							on:click={(_) =>
								goto(`/deployment/tasks?model_id=${model.id}&adapter_id=${adapter.id}`)}
							type="button"
