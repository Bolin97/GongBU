<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (showAdapters) {
      adapters = (await axios.get(`/api/adapter/by_base_model/${modelId}`))
        .data;
    }
  });
</script>
...
{#if model != null}
  ...
  >
    <div class="flex">
      ...
      <button
        class={`p-2 ${baseModelNoCursorChange ? "cursor-default" : ""} w-full rounded-md ${highlightBase ? "text-white bg-blue-400 border border-blue-800 shadow-lg" : ""}`}
      ...
      >
        ...
        <p
          class="mt-1 ${highlightBase ? 'text-white' : 'text-gray-800'} text-xs"
        >
    ...
    {#if showAdapters && adapters.length > 0}
      <div class="mx-4">
        <Accordion flush>
          <AccordionItem>
            <span slot="header">{t("components.model_card.adapters")}</span>
              ...
              <thead>
                ...
                >
                  ...
                  <th
                    class="px-4 py-2 bg-gray-200"
                    style="background-color:#f8f8f8"
                    >{t("components.model_card.name_and_description")}</th
                  >
                  {#if adapterAction}
                    <th
                      class="px-4 py-2 bg-gray-200"
                  ...
                  >
                    ...
                    <td class={`px-4 py-2 `}
                      ><div class="flex flex-col">
                        ...
                        <div
                          class={`text-xs ${adapter.id === highlightAdapter ? "text-white" : "text-gray-500"}`}
                        >
                    ...
                    {#if adapterAction}
                      <td class="px-4 py-2">
                        <slot name="adapter-action" {adapter} />
                      </td>
                    {/if}
                  </tr>
                {/each}
